...
(function( $, undefined ) {
  ...
  $.widget( "ui.addresspicker", {
    ...
    _create: function() {
      ...
      this.address1 = $(this.options.elements.address1);
      this.address2 = $(this.options.elements.address2);
    ...
    _updateAddressParts: function(geocodeResult){
      ...
      if (this.route != '') {
        this.address1.val(geocodeResult.value.replace(/(^[^,]+)(,.+$)/i,'$1').trim());

        if (geocodeResult.value.split(',').length > 3) {
            this.address2.val(geocodeResult.value.replace(/(^[^,]+,)([^,]+)(,.+$)/i,'$2').trim());
            }
        }
    },
